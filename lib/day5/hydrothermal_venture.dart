void main(List<String> args) {
  ...
  for (var item in generateListOfLines()) {
    allCoords.addAll(item.generateLine(includeDiagonal: true));
  }

  allCoords.sort((a, b) => b.x.compareTo(a.x));
  final maxX = allCoords.first.x;
  allCoords.sort((a, b) => b.y.compareTo(a.y));
  final maxY = allCoords.first.y;

  Map<String, int> occurences = {};
  for (var item in allCoords) {
    var key = item.toString();
    if (occurences.containsKey(key)) {
      occurences[key] = occurences[key]! + 1;
    } else {
      occurences.addAll({key: 1});
    }
  }

  print(occurences.values.where((element) => element > 1).length);
}
...
List<Line> generateListOfLines() {
  ...
  for (var item in input) {
    ...
    var startCoord = Coordinate(
        int.parse(startSplitted.first), int.parse(startSplitted.last));
    var endCoord =
        Coordinate(int.parse(endSplitted.first), int.parse(endSplitted.last));
    listOfLines.add(Line(startCoord, endCoord));
  }
  return listOfLines;
}
    ...
    if (start.x == end.x) {
      ...
      if (start.x < end.x && start.y < end.y) {
        for (var i = 0; i < diagonalLength; i++) {
          line.add(Coordinate(start.x + i, start.y + i));
        }
        ...
        for (var i = 0; i < diagonalLength; i++) {
          line.add(Coordinate(end.x - i, end.y + i));
        }
      }
