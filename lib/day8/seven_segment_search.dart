...
final mapOfSevenDisplayDigitSystem = {
  0: 'abcefg',
  1: 'cf',
  2: 'acdeg',
  3: 'acdfg',
...
void main(List<String> args) {
  print('total sum of all ${part2()}');
}

int countNumbers() {
  final nums = [1, 4, 7, 8];
  final numsLengths =
      nums.map((e) => mapOfSevenDisplayDigitSystem[e]?.length).toList();

  return input.fold<int>(
      0,
      (previousValue, element) =>
          previousValue +
          splitLine(splitLine(element, '|').last.trim(), ' ').fold<int>(
              0,
              (previousValue, element) =>
                  previousValue +
                  (numsLengths.contains(element.trim().length) ? 1 : 0)));
}
...
//   eee
int part2() {
  var finalRes = input.fold<int>(0, (previousValue, element) {
    var parts = splitLine(element, '|');
    var splitedLine = splitLine(parts.first, ' ');
    final mapOfNums = generateMap(splitedLine);
    var values = mapOfNums.values.toList();
    var keys = mapOfNums.keys.toList();
    return previousValue +
        int.parse(splitLine(parts.last.trim(), ' ').map((e) {
          return keys[values.indexOf(sortAlphabetically(e))];
        }).join());
  });
...
String sortAlphabetically(String input) {
  ...
  splitted.sort((a, b) => a.compareTo(b));
  return splitted.join();
}

bool containsAllElements(String input, String toCheck) {
  return !toCheck.split('').any((element) => !input.contains(element));
}

Map<int, String> generateMap(List<String> input) {
  var map = {
    1: sortAlphabetically(input.firstWhere((element) => element.length == 2)),
    4: sortAlphabetically(input.firstWhere((element) => element.length == 4)),
    7: sortAlphabetically(input.firstWhere((element) => element.length == 3)),
    8: sortAlphabetically(input.firstWhere((element) => element.length == 7)),
  };

  input.where((st) => st.length == 5).forEach((element) {
    var sortedThreePotential = sortAlphabetically(element);
    if (containsAllElements(sortedThreePotential, map[1]!) &&
        containsAllElements(sortedThreePotential, map[7]!)) {
      map[3] = sortedThreePotential;
    } else if ((sortedThreePotential + map[4]!).split('').toSet().length == 7) {
      map[2] = sortedThreePotential;
    } else {
      map[5] = sortedThreePotential;
    }
  });

  input.where((st) => st.length == 6).forEach((element) {
    var sortedThreePotential = sortAlphabetically(element);
    if ((sortedThreePotential + map[1]!).split('').toSet().length == 7) {
      map[6] = sortedThreePotential;
    } else if (containsAllElements(sortedThreePotential, map[5]!)) {
      map[9] = sortedThreePotential;
    } else {
      map[0] = sortedThreePotential;
    }
  });
...
List<String> splitLine(String toSplit, String delimiter) {
